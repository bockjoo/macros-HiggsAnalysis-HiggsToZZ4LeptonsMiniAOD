  ...
  for ff in flist:
    ...
    for n in range(0, len(weight)):
      ...
      for i in range(0, len(cut_m4l)):     
         #if np.abs(mass4l[n] - 125) < cut_m4l[i] and 'Run2016' not in f.GetName() and 'MZp' not in f.GetName(): 
         #    counter_B[i] += weight[n]
         #if np.abs(mass4l[n] - 125) < cut_m4l[i] and signal in f.GetName():
